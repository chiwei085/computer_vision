...
[[nodiscard]] inline std::uint8_t clamp_to_u8(double value) {
...
template <detail::kernel_container Kernel, class Padding = zero_padding>
requires std::convertible_to<detail::kernel_value_t<Kernel>, double>&&
    ...
    // The default strategy is zero padding: pixels outside the image bounds
    // are treated as 0, which preserves the output size and allows edge pixels
    // to be convolved with the same kernel shape as interior pixels.
