...
concept kernel_container =
    std::ranges::sized_range<Kernel> &&
    requires(const Kernel& kernel) { std::data(kernel); };
...
template <detail::kernel_container Kernel, class Padding = reflect_101_padding>
    requires std::convertible_to<detail::kernel_value_t<Kernel>, double> &&
             std::regular_invocable<Padding&, ImageView<const Gray8>, int,
                                    int> &&
             std::convertible_to<
                 std::invoke_result_t<Padding&, ImageView<const Gray8>, int,
                                      int>,
                 Gray8>
inline void conv(ImageView<const Gray8> src, ImageView<Gray8> dst,
    ...
    requires std::convertible_to<detail::kernel_value_t<Kernel>, double> &&
             ...
             std::convertible_to<
                 std::invoke_result_t<Padding&, ImageView<const Gray8>, int,
                                      int>,
                 Gray8>
[[nodiscard]] inline Image<Gray8> conv(ImageView<const Gray8> src,
                                       const Kernel& kernel, int kernel_width,
                                       int kernel_height, int stride_x = 1,
