...
inline void update_histogram_region(Histogram& hist,
                                    ImageView<const Gray8> src, int row_begin,
                                    int row_end, int col_begin, int col_end,
                                    Padding& padding, int delta) {
    ...
    for (int y = 0; y < dst.height(); ++y) {
        if (y > 0) {
            ...
            update_histogram_region(
                row_hist, src, old_row_begin, std::min(new_row_begin, old_row_end),
                first_col_begin, first_col_end, padding, -1);
            update_histogram_region(
                row_hist, src, std::max(old_row_end, new_row_begin), new_row_end,
                first_col_begin, first_col_end, padding, +1);
        }
        ...
        for (int x = 0; x < dst.width(); ++x) {
            ...
            update_histogram_region(
                hist, src, row_begin, row_end,
                std::max(old_col_end, new_col_begin), new_col_end, padding, +1);
        }
    }
}
...
inline void median_filter(ImageView<const Gray8> src, ImageView<Gray8> dst,
    ...
    if constexpr (Radius <= detail::median_small_radius_threshold) {
        detail::median_filter_rebuild_window<Radius>(src, dst, pad_x, pad_y,
                                                     stride_x, stride_y,
                                                     padding);
    }
    else {
